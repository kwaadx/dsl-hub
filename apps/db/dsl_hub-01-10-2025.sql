...
create trigger thread_set_archived_ts
    before update on thread
    for each row execute function set_archived_ts();
...
create table if not exists generation_run (
                                              ...
                                              constraint generation_run_stage_ck  check (stage in ('discovery','generate','self_check','hard_validate','publish')),
...
create trigger schema_upgrade_plan_set_updated_at
    before update on schema_upgrade_plan
    for each row execute function set_updated_at();
